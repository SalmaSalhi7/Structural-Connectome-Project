...
def analyze(run, subject):
    ...
    aseg_dir = '/Volumes/Extreme SSD/Brain/Toshiba4TB/' + subject
    ...
    for e in endpoints_label_map:
        #if e[0] not in [2,41,7,46,0,16]: # 41 = Right Cerebral White Matter
        if e[0] in all_labels:
            #if e[1] not in [2,41,7,46,0,16]:
            if e[1] in all_labels:
                label_map[e[0]][e[1]] += 1
...
for subject in subjects:
    for run in p:
        ...
        title = '/Volumes/Extreme SSD/Brain/Toshiba4TB/' + subject + 'bi-directional_label_map-shorder=6-maxangle=45-gfa=0.25-subject=' + subject.split(None, 1)[0] + run +'.pkl'  
