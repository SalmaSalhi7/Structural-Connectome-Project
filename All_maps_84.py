...
aseg_dir = '/Volumes/Extreme SSD/Brain/Toshiba4TB/' + subjects[0] 
...
def analyze(run, subject):
    ...
    data_file = '/Volumes/Extreme SSD/Brain/Toshiba4TB/' + subject + 'bi-directional_label_map-shorder=6-maxangle=45-gfa=0.25-subject=' + subject.split(None, 1)[0] + run +'.pkl'
    ...
    labels_id = pd.DataFrame([i for i in data[8].keys()],  columns = ['id'])

    # a dataframe for structure names
    labels_name = pd.DataFrame([id_to_name[i] for i in data[8].keys()],  columns = ['name']) 
    ...
    remove = [2,4,5,7,14,15,16,24,28,30,31,41,43,44,46,60,62,63,72,77,78,79,80,81,82,85,251,252,253,254,255,1000,1004,2000,2004]
    msk = Master['id'] == 0
...
for subject in subjects:
    ...
    for run in p:
        ...
        MPM.to_csv('/Volumes/Extreme SSD/Brain/Toshiba4TB/' + subject + 'Connectivity_84_' + subject.split(None, 1)[0] + run + '.csv')
