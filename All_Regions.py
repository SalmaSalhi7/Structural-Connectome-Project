...
def Analyze(img_d_path,img_s_path,gtab, run, subject):
    ...
    # making an empty mask for the corpus callosum with the same shape as the 3D reduced_size_label array
    CC = np.zeros(reduced_size_label.shape)

    # NOTE ON BOOLEAN OPERATIONS: the boolean operation is performed first. Comparing reduced_size_label with i returns a boolean matrix that has True values at the indices where the value in the matrix is equal to i. Then, the += operator adds this matrix
    # to left_cortex (boolean addition)

    # populating the corpus callosum mask with all CC regions using corresponding labels
    ...
    for i in [4,5,8,10,11,12,13,14,15,16,17,18,24,26,28,30,31,43,44,47,49,50,51,52,53,54,58,60,62,63,85]: 
    ...
    print('Probabalistic direction getter complete')
    ...
    print('Streamlines generator complete')
...
for subject in subjects:
    ...
    for run in p:
        Analyze(path + subject + datafile_d, path + subject + datafile_aparc_aseg, gtab, '', subject)
        print('Done ', run)
